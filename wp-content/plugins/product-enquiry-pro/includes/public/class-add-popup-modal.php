...
class QuoteUpAddPopupModal
{
    ...
    public function checkApproved($enquiry_id)
    {
        ...
        if (!empty($appRejLink)) {
            ?>
            <div>
                <a href="<?php echo $appRejLink['approve']; ?>" class="reject-quote-button" target="_blank"> <?php _e('Approve', QUOTEUP_TEXT_DOMAIN) ?></a> |
                <a href="<?php echo $appRejLink['reject']; ?>" class="reject-quote-button" target="_blank">  <?php _e('Reject', QUOTEUP_TEXT_DOMAIN) ?></a>
            </div>
            <?php
        }
    }
