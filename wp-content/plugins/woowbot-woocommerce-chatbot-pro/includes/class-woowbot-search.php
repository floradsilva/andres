...
if ( ! class_exists( 'WoowBot_Search' ) ) :
    /**
     * Class for plugin search
     */
    class WoowBot_Search {
        /**
        ...
        public function search( $keyword = ''  ) {
            ...
            $tags_array = array();
            $this->data['s'] = $s;
            $this->data['results_num']  =  50;//This static before setting option on woowbot admin panel.
       ...
       public function get_special_chars() {
        ...
        private function query_index_table() {
            global $wpdb;
            $table_name = $wpdb->prefix . QCLD_WOOCHATBOT_INDEX_TABLE;
			...
			$search_term_count = count($this->data['search_terms']);
            ...
            foreach ( $this->data['search_terms'] as $search_term ) {
				...
				$relevance_title        = 1 ;
                $relevance_content      = 1 ;
                $relevance_title_like   = 1 ;
                $relevance_content_like = 1 ;
                ...
                foreach ( $search_in_arr as $search_in_term ) {
                    switch ( $search_in_term ) {
                        case 'title':
                            $relevance_array['title'][] = $wpdb->prepare( "( case when ( term_source = 'title' AND term = '%s' ) then {$relevance_title}  else 0 end )", $search_term );
                            $relevance_array['title'][] = $wpdb->prepare( "( case when ( term_source = 'title' AND term LIKE %s ) then {$relevance_title_like}  else 0 end )", $like );
                            break;
                        case 'content':
                            $relevance_array['content'][] = $wpdb->prepare( "( case when ( term_source = 'content' AND term = '%s' ) then {$relevance_content}  else 0 end )", $search_term );
                            $relevance_array['content'][] = $wpdb->prepare( "( case when ( term_source = 'content' AND term LIKE %s ) then {$relevance_content_like}  else 0 end )", $like );
                            break;
                        case 'excerpt':
                           // $relevance_array['content'][] = $wpdb->prepare( "( case when ( term_source = 'excerpt' AND term = '%s' ) then {$relevance_content}  else 0 end )", $search_term );
                           // $relevance_array['content'][] = $wpdb->prepare( "( case when ( term_source = 'excerpt' AND term LIKE %s ) then {$relevance_content_like}  else 0 end )", $like );
                            break;
                        case 'category':
                            //$relevance_array['category'][] = $wpdb->prepare( "( case when ( term_source = 'category' AND term = '%s' ) then 1 else 0 end )", $search_term );
                            //$relevance_array['category'][] = $wpdb->prepare( "( case when ( term_source = 'category' AND term LIKE %s ) then 1 else 0 end )", $like );
                            break;
                        case 'tag':
                            $relevance_array['tag'][] = $wpdb->prepare( "( case when ( term_source = 'tag' AND term = '%s' ) then 1 else 0 end )", $search_term );
                            $relevance_array['tag'][] = $wpdb->prepare( "( case when ( term_source = 'tag' AND term LIKE %s ) then 1 else 0 end )", $like );
                            break;
                        case 'sku':
                           // $relevance_array['sku'][] = $wpdb->prepare( "( case when ( term_source = 'sku' AND term = '%s' ) then 1 else 0 end )", $search_term );
                           // $relevance_array['sku'][] = $wpdb->prepare( "( case when ( term_source = 'sku' AND term LIKE %s ) then 1 else 0 end )", $like );
                            break;
                    }
                }
            }
            ...
            //$query['search'] .= sprintf( ' AND ( %s )', implode( ' AND ', $search_array ) );
           // $query['source'] .= sprintf( ' AND ( %s )', implode( ' AND ', $source_array ) );
            ...
            $sql = "SELECT
				...
				LIMIT 0, {$results_num}
		    ";
           //ORDER BY relevance DESC // HAVING  relevance/$search_term_count > 1.5
		   //echo $sql; 
