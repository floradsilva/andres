...
class Calendar extends ProCalendar
{
            ...
            do {
                ...
                foreach ( $events->getItems() as $event ) {
                    if ( $this->_isDeletedEvent( $event ) || $this->_isTransparentEvent( $event ) ) {
                        ...
                        $this->_populateAppointment( $appointment ?: new Appointment(), $event )->save();
                    }
                }
    ...
    {
        $start_date = DatePoint::fromStr( sprintf( '-%d days midnight', get_option( 'bookly_gc_full_sync_offset_days', 0 ) ) );
        ...
        if ( $this->_deleteEventsCreatedFromBookly( $start_date ) ) {
            // 3. Copy Google Calendar events to appointments.
            if ( $this->_copyEventsToAppointments( $start_date ) ) {
     ...
     * @return bool
     */
    protected function _copyEventsToAppointments( DatePoint $start_date )
    {
        try {
            $params = array(
                'singleEvents' => true,
            ...
            do {
                ...
                foreach ( $events->getItems() as $event ) {
                    if ( ! $this->_isTransparentEvent( $event ) ) {
                        $this->_populateAppointment( new Appointment(), $event )->save();
                    }
                }
                $params['pageToken'] = $events->getNextPageToken();
     ...
     * @return bool
     */
    protected function _deleteEventsCreatedFromBookly( DatePoint $start_date )
    {
        try {
            $params = array(
