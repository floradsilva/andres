...
function wc_instagram_connect( $args ) {
	...
	$args = wp_parse_args(
		...
		array(
			'access_token' => '',
			'expires_at'   => strtotime( '+90 days' ), // The access token is valid during 90 days by default.
		)
	);
...
function wc_instagram_disconnect() {
	...
	// Remove the scheduled access renewal.
...
function wc_instagram_schedule_access_renewal() {
	...
	if ( $expires_at > $current_time ) {
		...
		// Today was the last day and the access couldn't be renewed.
...
/**
 * Unschedules the event for renewing the Instagram access credentials.
 *
 * @since 2.1.0
 */
