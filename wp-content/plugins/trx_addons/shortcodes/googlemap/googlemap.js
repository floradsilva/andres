			...
			.each(function () {
				...
				map.find('.sc_googlemap_marker').each(function() {
					...
					map_markers.push({
						...
						offsetX:		marker.data('offsetx'),
						offsetY:		marker.data('offsety')
					});
				});
	...
	for (var i in TRX_ADDONS_STORAGE['googlemap_init_obj'][id].markers) {
		...
		if (TRX_ADDONS_STORAGE['googlemap_init_obj'][id].markers[i].latlng == '') {
			...
			if (TRX_ADDONS_STORAGE['googlemap_init_obj'][id].opt.center == null) {
                ...
                var scale = Math.pow(2, TRX_ADDONS_STORAGE['googlemap_init_obj'][id].map.getZoom());
                if(screen.width<1262 && screen.width>959){
                    scale = scale * 1.5;
				} else if(screen.width<959 && screen.width>767){
                    scale = scale * 2.25;
                }
                var worldCoordinateCenter = TRX_ADDONS_STORAGE['googlemap_init_obj'][id].map.getProjection().fromLatLngToPoint(markerInit.position);
                var pixelOffset = new google.maps.Point((TRX_ADDONS_STORAGE['googlemap_init_obj'][id].markers[i].offsetX/scale) || 0,(TRX_ADDONS_STORAGE['googlemap_init_obj'][id].markers[i].offsetY/scale) ||0);
                ...
                var newCenter = TRX_ADDONS_STORAGE['googlemap_init_obj'][id].map.getProjection().fromPointToLatLng(worldCoordinateNewCenter);
				TRX_ADDONS_STORAGE['googlemap_init_obj'][id].map.setCenter(newCenter);
			}
