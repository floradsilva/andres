			...
			jQuery.each(result, function(email_value, data_array)
			{
				...
				jQuery.each(data_array, function( key, value )
				{
					if(was_already_defined)
					{
						...
						{
							value += typeof prev_value[12] !== 'undefined' ? prev_value[12] : 0; //See the definition of "csv_data_columns" to know to which column the index is associated with
						}
						else if(key == 'orders')
						{
							value += prev_value[11];
			...
			jQuery.each(guest_customers, function( key, value_array )
				{
					...
					if(typeof value_array !== 'string')
					{
						...
						total_spent = total_spent.countDecimals() > 2 ? total_spent.toFixed(2) : total_spent;
