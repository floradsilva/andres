	...
	function process_csv_upload_ajax()
	{
		...
		$this->process_uploaded_file($csv_array);
		 ...
		 if ( isset($this->errors) && count($this->errors) > 0):  ?>
			...
			if ( ! empty( $this->updated_user_list ) ) 
			{	echo '<div id="message" class="updated"><p>'.__( 'Updated users: ', 'woocommerce-customers-manager' );
					foreach ( $this->updated_user_list as $user_data )
						 echo $user_data['id']." ".$user_data['username']/* ." ".$user_data['username_existed'] */."; ";
			...
			if($csv_array != null)
			{
				...
				foreach($csv_array as $csv_row)
				{
					...
					if($user != null)
					{
						...
						$username_existed = $user_id = username_exists( $user[ 'Login' ] );
						$is_valid_id = isset($user[ 'ID' ]) ? is_numeric($user[ 'ID' ]) : true;
						if($is_valid_id)
						{
							$wp_user = get_userdata( $user[ 'ID' ]);
							$user_id = ( $wp_user === false ) ? false : $user[ 'ID' ]; //It could happen that username are associated to different ID when importing
						}
						...
						if($is_valid_id && $user_id != false)
						{
							//if($mail_exists_id == false || ($mail_exists_id == $user_id) || (isset($user[ 'ID' ]) && $mail_exists_id == $user[ 'ID' ]))
							{
								...
								$this->updated_user_list[] = array('username' => $user[ 'Login' ], 'id' => $user_id, 'username_existed'=>$username_existed);
