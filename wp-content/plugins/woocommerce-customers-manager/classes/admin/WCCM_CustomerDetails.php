	...
	?>
		<div id="user-general-details">
			...
			<p>
				...
				<?php 
				$user_total_spent = $wccm_customer_model->get_user_total_spent(null,null, $this->get_all_user_orders($this->current_customer_id)/* $orders */);
				$user_total_spent_with_refunds = $wccm_customer_model->get_user_total_spent(null,null, $this->get_all_user_orders($this->current_customer_id), true, true);
				
				?>
				<label><?php  _e('Total Spent', 'woocommerce-customers-manager'); ?> </label> <b><?php echo $user_total_spent; ?></b> <br/><br/>
				<?php if($user_total_spent != $user_total_spent_with_refunds): ?>
					<label><?php  _e('Total Spent including refunds', 'woocommerce-customers-manager'); ?> </label> <b><?php echo $user_total_spent_with_refunds; ?></b> <br/><br/>
				<?php endif; ?>
			</p>
					...
					<div class="order-block">
						...
						<?php 
							...
							if($order_status == 'COMPLETED')
							  ...
							  //$order->post_status; 
						$refunded = $wc_order->get_total_refunded();
						$refunded = isset($refunded) ? floatval($refunded):0;
						if(method_exists ($wc_order,'get_total_shipping'))
						{
						 ...
						 $total_order = get_woocommerce_currency_symbol().(round($order_total+$taxes_total+$total_shipping_numeric+$wc_order->get_shipping_tax()-$wc_order->get_total_discount(false) - $refunded,2));
						...
						<span class="stats"><span class="stats_title"><?php _e('Total refunded:', 'woocommerce-customers-manager' ); ?></span> <span class="stats_content"><?php echo get_woocommerce_currency_symbol().($refunded); ?></span></span>
