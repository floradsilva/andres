 ...
 * @since 1.0
 */
class WC_Constant_Contact_API extends Framework\SV_WC_API_Base {
	 ...
	 *
	 * If a list is not provided, the default list set in WooCommerce > Settings > Constant Contact will be used.
	 *
	 * @since 1.0
	 *
	 ...
	 * This method will sometimes return a feed list without any entries, or error 500, which is why the exception is caught and checked inside the method.
	 ...
	 * @return bool
	 * @throws Framework\SV_WC_API_Exception rethrows previous exception if not a 500 error code
	 */
	 ...
	 * @throws Framework\SV_WC_API_Exception required data missing
	 */
	 ...
	 *
	 * Due to limitations with the v1 API, this only includes total subscribers at the moment, but could be easily extended to include more info as the API improves
	 *
	 * GET /lists
	 *
	 * @since 1.0
	 ...
	 * @throws Framework\SV_WC_API_Exception required data is missing
	 */
	...
	public function subscribe_customer( $order_id ) {
		...
		if ( $this->contact_exists( $billing_email ) ) {
			...
			/**
			 * Filters new contact data.
			 *
			 * @since 1.0.0
			 *
			 * @param array $customer_data the customer data for the new contact
			 * @param \WC_Order $order the current order for the customer
			 */
			$customer_data = apply_filters( 'wc_constant_contact_new_contact_data', array(
				'FirstName'   => Framework\SV_WC_Order_Compatibility::get_prop( $order, 'billing_first_name' ),
				'LastName'    => Framework\SV_WC_Order_Compatibility::get_prop( $order, 'billing_last_name' ),
				'Addr1'       => Framework\SV_WC_Order_Compatibility::get_prop( $order, 'billing_address_1' ),
				'Addr2'       => Framework\SV_WC_Order_Compatibility::get_prop( $order, 'billing_address_2' ),
				...
				'PostalCode'  => Framework\SV_WC_Order_Compatibility::get_prop( $order, 'billing_postcode' ),
				'CountryCode' => Framework\SV_WC_Order_Compatibility::get_prop( $order, 'billing_country' ),
			), $order );
	...
	protected function do_pre_parse_response_validation() {
		...
		if ( ! in_array( $this->get_response_code(), array( '200', '201', '204' ), false ) ) {
			throw new Framework\SV_WC_API_Exception( sprintf( __( 'Error Code %s', 'woocommerce-constant-contact' ), $this->get_response_code() ), $this->get_response_code() );
		}
	}
