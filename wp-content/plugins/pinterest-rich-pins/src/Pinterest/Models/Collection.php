...
use Vsourz\Pinterest\Exceptions\PinterestException;

use \Vsourz\Pinterest\Pinterest;
use \Vsourz\Pinterest\Transport\Response;
...
class Collection implements \JsonSerializable, \ArrayAccess, \IteratorAggregate{
    ...
    public function __construct(Pinterest $master, $items, $model) {
        ...
        if (!class_exists("\\Vsourz\\Pinterest\\Models\\" . $this->model)) {
        ...
        } else if ($items instanceof \Vsourz\Pinterest\Transport\Response) {
            $this->response = $items;
            $this->items = $items->data;
        } else {
    ...
    private function buildCollectionModels(array $items)
    {
        ...
        foreach ($items as $item) {
            $class = new \ReflectionClass("\\Vsourz\\Pinterest\\Models\\" . $this->model);
     ...
     * @return string
     */
    public function toJson($isObj = false)
    {
