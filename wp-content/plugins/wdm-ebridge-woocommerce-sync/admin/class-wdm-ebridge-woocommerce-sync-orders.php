...
if ( ! class_exists( 'Wdm_Ebridge_Woocommerce_Sync_Orders' ) ) {
	class Wdm_Ebridge_Woocommerce_Sync_Orders {
		...
		public function wews_create_order( $order, $data ) {
			...
			if ( ! $customer ) {
				throw new Exception( __( 'Unable to place this order. Please enter valid billing phone and email.', 'wdm-ebridge-woocommerce-sync' ) );
			}
		...
		public function find_or_create_ebridge_customers( $order, $data ) {
			...
			if ( ! $customer ) {
				$customer = $this->customer_obj->create_ebridge_customer( $data );
			}
			// }
		...
		public function map_order_data( $ebridge_order, $order ) {
			$order->add_meta_data( '_ebridge_order_id', $ebridge_order );
		}
	}
