...
class Lifecycle extends Framework\Plugin\Lifecycle {
	...
	protected function install() {
		...
		// performs migration for the installed legacy plugin
		// TODO: determine the best order here, since only one gateway should be migrated
		if ( $this->is_aim_installed() ) {
	 ...
	 * @since 3.0.0-dev
	 */
	private function migrate_from_aim() {
		...
		$this->migrate_credit_card_gateway( 'authorize_net_aim', $cc_settings );
		...
		$this->migrate_echeck_gateway( 'authorize_net_aim_echeck', $echeck_settings );
	 ...
	 * @since 3.0.0-dev
	 */
	private function migrate_from_sim() {
		...
		if ( 'sim' === get_option( 'wc_authorize_net_sim_active_integration' ) ) {
			...
			$this->migrate_credit_card_gateway( 'authorize_net_sim_credit_card', $cc_settings );
			...
			$this->migrate_echeck_gateway( 'authorize_net_sim_echeck', $echeck_settings );
		...
		// otherwise, migrate accept hosted
			...
			$this->migrate_credit_card_gateway( 'authorize_net_accept_hosted_credit_card', $cc_settings );
			...
			$this->migrate_echeck_gateway( 'authorize_net_accept_hosted_echeck', $echeck_settings );
	 ...
	 */
	private function migrate_credit_card_gateway( $old_gateway_id, array $force_settings = [] ) {

		// don't overwrite existing CIM settings
		if ( ! empty( $this->get_credit_card_settings() ) ) {
	 ...
	 * @param array $force_settings settings to override to a specific value
	 */
	private function migrate_echeck_gateway( $old_gateway_id, array $force_settings = [] ) {

		// don't overwrite existing CIM settings
		if ( ! empty( $this->get_echeck_settings() ) ) {
	 ...
	 * @since 3.0.0-dev
	 */
	private function migrate_from_dpm() {
		global $wpdb;
