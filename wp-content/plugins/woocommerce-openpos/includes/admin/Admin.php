        ...
        if($pos_custom_item == 'yes' && $pos_tax_class == 'op_productax')
        {
            ...
            if( $pos_custom_item_tax_class != 'op_notax')
            {
                ...
                if( !empty($rate_options))
                {
                    ...
                    $pos_custom_item_tax_rate_setting = array(
                        'name'              => 'pos_custom_item_tax_rate',
                        'label'             => __( 'Custom Item Tax Rate', 'openpos' ),
                        'desc'              => __('Add custom item tax rate, this rate for custom item on POS only','openpos'),
                        'default'           => '0',
                        'type'              => 'select',
...
//            )
    ...
    public function products()
    {
        ...
        foreach($posts_array as $post)
        {
            ...
            $allow_types = apply_filters('op_allow_product_type',$allow_types,$_product);
    ...
    public function admin_style() {
        ...
        if(in_array($current_page,$all_pos_page))
        {
            ...
            if($current_page == 'openpos-dasboard' || $current_page == 'op-reports')
            {
                wp_enqueue_script('gstatic.chart', 'https://www.gstatic.com/charts/loader.js');
                wp_add_inline_script('gstatic.chart', "google.charts.load('current', {'packages':['corechart','table']});");
            }
    ...
    function sessions_page() {
        ...
        foreach($sessions as $key => $s)
        {
            ...
            $tmp = array(
                $s['session'],
                $s['username'],
                $this->core->convertToShopTime($s['logged_time']),
                $ip,
                $_register_location,
                $location,
    ...
    public function export_inventory(){
        ...
        fputcsv( $fh, array(__('Barcode','openpos'),__('Qty','openpos')));
        //start product data
