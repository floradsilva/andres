...
if (!function_exists('gravity_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'gravity_hovers_theme_setup3', 3 );
	function gravity_hovers_theme_setup3() {
		// Add 'Buttons hover' option
		gravity_storage_set_array_before('options', 'sidebar_widgets', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'gravity'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'gravity') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'gravity'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'gravity'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'gravity') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'gravity'),
					'icon'	=> esc_html__('Icon',	'gravity'),
					'icons'	=> esc_html__('Icons',	'gravity'),
					'zoom'	=> esc_html__('Zoom',	'gravity'),
					'fade'	=> esc_html__('Fade',	'gravity'),
					'slide'	=> esc_html__('Slide',	'gravity'),
					'pull'	=> esc_html__('Pull',	'gravity'),
					'border'=> esc_html__('Border',	'gravity')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('gravity_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'gravity_hovers_theme_setup9', 9 );
	function gravity_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'gravity_hovers_frontend_scripts', 1010 );
		add_filter( 'gravity_filter_localize_script','gravity_hovers_localize_script' );
		add_filter( 'gravity_filter_merge_scripts',	'gravity_hovers_merge_scripts' );
		add_filter( 'gravity_filter_merge_styles',	'gravity_hovers_merge_styles' );
		add_filter( 'gravity_filter_get_css', 		'gravity_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'gravity_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'gravity_hovers_frontend_scripts', 1010 );
	function gravity_hovers_frontend_scripts() {
		if ( gravity_is_on(gravity_get_theme_option('debug_mode')) && gravity_get_file_dir('includes/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'gravity-hovers', gravity_get_file_url('includes/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( gravity_is_on(gravity_get_theme_option('debug_mode')) && gravity_get_file_dir('includes/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'gravity-hovers',  gravity_get_file_url('includes/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('gravity_hovers_merge_scripts')) {
	//Handler of the add_filter( 'gravity_filter_merge_scripts', 'gravity_hovers_merge_scripts' );
	function gravity_hovers_merge_scripts($list) {
...
if (!function_exists('gravity_hovers_merge_styles')) {
	//Handler of the add_filter( 'gravity_filter_merge_styles', 'gravity_hovers_merge_styles' );
	function gravity_hovers_merge_styles($list) {
...
if (!function_exists('gravity_hovers_localize_script')) {
	//Handler of the add_filter( 'gravity_filter_localize_script','gravity_hovers_localize_script' );
	function gravity_hovers_localize_script($arr) {
		$arr['button_hover'] = gravity_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('gravity_hovers_add_icons') ) {
	function gravity_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'gravity') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink());; ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') esc_html_e('Details', 'gravity');
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'gravity_hovers_get_css' ) ) {
	//Handler of the add_filter( 'gravity_filter_get_css', 'gravity_hovers_get_css', 10, 4 );
	function gravity_hovers_get_css($css, $colors, $fonts, $scheme='') {
