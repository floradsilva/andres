...
<div class="sc_services_item<?php
    ...
    echo !empty($args['inverse_colors']) && $args['inverse_colors']==1 ? ' sc_services_item_inverse_colors' : ' ';
?>">
	<?php
	...
	if ( has_post_thumbnail() && (empty($args['featured']) || $args['featured']=='image')) {
		...
		?><a href="<?php echo esc_url($link); ?>"
			 id="<?php echo esc_attr($args['id'].'_'.trim($meta['icon']=='No icon'?rand(0,100):$meta['icon'])); ?>"
			 class="sc_services_item_icon <?php echo empty($svg) ? esc_attr($meta['icon']=='No icon'?'no_icon':$meta['icon']) : 'sc_icon_type_svg'; ?>"
			 ><?php
			if (!empty($svg)) {
				gravity_show_layout(trx_addons_get_svg_from_file($svg));
			}
		?></a><?php
	...
	<div class="sc_services_item_info">
		<div class="sc_services_item_header">
			...
			if ($args['featured']=='number' && $featured_position=='top') {
				?><div class="sc_services_item_subtitle"><?php gravity_show_layout(trx_addons_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_SERVICES_TAXONOMY));?></div><?php
			}
			?>
		...
		<?php if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) { ?>
			...
			<div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="sc_button sc_button_simple"><?php esc_html_e('Details', 'gravity'); ?></a></div>
		<?php } ?>
	</div>
</div>
<?php
