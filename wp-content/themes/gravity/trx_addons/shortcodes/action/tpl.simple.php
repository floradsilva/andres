...
?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> 
	...
	echo (trim($args['columns'] > 1 ? ' data-slides-per-view="' . esc_attr($args['columns']) . '"' : '')
	...
	foreach ($args['actions'] as $item) {
		...
		?>>
			...
			if (!empty($item['image'])) {
				...
				?><div class="sc_action_item_image"><img src="<?php echo esc_url($item['image']); ?>" <?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
			} else {
			...
			?> <div class="sc_action_item_header"><?php
			...
			if (!empty($item['description'])) {
				...
				?><div class="sc_action_item_description"><?php
					foreach ($item['description'] as $str) {
						?><span><?php gravity_show_layout($str); ?></span><?php
					}
				?></div><?php
			}
            ?></div><?php
